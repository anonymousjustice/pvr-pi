...
bool OGLES2PhantomMask::RenderScene()
{
	...
	*/
	...
	unsigned int aspect[2];
	m_Print3D.GetAspectRatio(&aspect[0], &aspect[1]);

	float fY = (float)(int(m_Print3D.GetFontLineSpacing() / (480.0f / 100.0f)) / ((float) aspect[1] / 480.0f));
	fY *= 1.03f;

  
	// Shadow
	m_Print3D.Print3D(0.5f, 0.03f, 1.03f,  PVRTRGBA(0,0,0,255), "PhantomMask");

	if(m_bEnableSH)
	{
		// Shadow        
		m_Print3D.Print3D(0.5f, fY, 0.803f,  PVRTRGBA(0,0,0,255), "Spherical Harmonics Lighting");
	...
	{
		// Shadow
		m_Print3D.Print3D(0.5f, fY, 0.803f,  PVRTRGBA(0,0,0,255), "Vertex Lighting");

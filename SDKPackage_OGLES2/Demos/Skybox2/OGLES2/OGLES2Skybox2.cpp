...
bool OGLES2Skybox2::InitView()
{
	...
	*/
...

	{
		...
		m_pEffects = new SEffect[g_ui32NoOfEffects];
...
bool OGLES2Skybox2::RenderScene()
{
	...
	*/
	...
	for(i = 0; i < m_Scene.nNumMeshNode; i++)
	{
		...
		location = glGetUniformLocation(m_pEffects[m_i32Effect].pEffect->GetProgramHandle(), "myEyePos");

		if(location != -1)
			glUniform3f(location, vCameraPosition.x, vCameraPosition.y, vCameraPosition.z);

		//set animation
		location = glGetUniformLocation(m_pEffects[m_i32Effect].pEffect->GetProgramHandle(), "fAnim");

		if(location != -1)
			glUniform1f(location, fBurnAnim);

		location = glGetUniformLocation(m_pEffects[m_i32Effect].pEffect->GetProgramHandle(), "myFrame");
		...
		if(g_bBlendShader[m_i32Effect])
		{
			...
			location = glGetUniformLocation(m_pEffects[m_i32Effect].pEffect->GetProgramHandle(), "bBackFace");
...
void OGLES2Skybox2::DrawSkybox()
{
	...
	{
		// Set Data Pointers
		glVertexAttribPointer(m_pEffects[0].psUniforms[iVertexUniform].nLocation, 3, GL_FLOAT, GL_FALSE, sizeof(float)*3, &((float*)0)[i*4*3]);

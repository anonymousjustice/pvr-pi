...
namespace pvrengine
{
			...
			MaterialManager *pMaterialManager)
	{
		...
		Texture* pTexture = m_pMaterialManager->getTextureManager()->LoadTexture(strTextureFile);
...
next:;
	...
	{
		// set context
		m_sEffect.SetContext(m_pUniformHandler->getContext());
	...
	bool Material::loadPFXTextures(const CPVRTString& strTexturePath)
	{
		...
		{
			CPVRTString strTexture(strTexturePath);

			if(strTexture.length())
				strTexture += "/";

			strTexture+=(char*)psTex[u32CurrentTexture].p;

			ConsoleLog::inst().log("Loading Texture: '%s'\n",strTexture.c_str());
			Texture* pTexture = m_pMaterialManager->getTextureManager()->LoadTexture(strTexture);
			if(!pTexture)
			{
	...
	/******************************************************************************/
	SPVRTContext* Material::getContext()
	{
		return m_pUniformHandler->getContext();
	}
}

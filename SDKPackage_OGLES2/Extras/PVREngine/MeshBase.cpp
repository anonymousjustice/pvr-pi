...
namespace pvrengine
{
	...
	{
		m_eDrawMode = eNormal;
		m_eGeometryPrimitiveType = eTriangles;
	}
	...
	bool Mesh::setDrawMode(const EDrawMode eDrawMode)
	{
		...
		//case eDepthComplexity:
			if(m_eGeometryPrimitiveType!=eTriangles)
			{
		...
		case eWireframe:
		case eWireframeNoFX:
			if(m_eGeometryPrimitiveType!=eLines)
			{
				if(!ConvertToLines())
				{
					return false;
	...
	bool Mesh::ConvertToLines()
	{
		...
		if(!m_psMesh->nNumStrips)
		{
			if(m_psMesh->sFaces.pData)
			{
				...
				PVRTuint16* pNewIndices = (unsigned short*)malloc(u32IndexCount*sizeof(PVRTuint16));
				if(!pNewIndices)
				{
				...
				PVRTuint16* pSrcPointer = (PVRTuint16*)m_psMesh->sFaces.pData,
		...
		m_eGeometryPrimitiveType = eLines;
		return true;
	}
	...
	void Mesh::DoBoundaries()
	{
		m_BoundingBox = BoundingBox(m_psMesh->pInterleaved,
			m_psMesh->nNumVertex,
			(int)(size_t)m_psMesh->sVertex.pData,

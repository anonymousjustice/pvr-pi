	...
	*****************************************************************************/
	bool append(T addT)
	{
		...
		{	// expand array: expensive
			...
			if(!pNewArray)
			{
				return false;	// failure to expand array means failure to append
			}
	...
	it if necessary. This is always expensive due to array storage method.
	*****************************************************************************/
	void prepend(T addT)
	{
		if(m_i32Size>=m_i32Capacity)
		{	// expand array: expensive
			...
			// copy all to new array leaving a space at the start
			for(int i=0;i<m_i32Size;++i)
			{
		...
		{
			// copy all in place leaving a space at the start
			for(int i=m_i32Size;i>0;--i)
			{
